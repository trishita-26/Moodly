        ...
        <div className='mt-6'>
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'> 
            {["posts","media","likes"].map((tab)=>(
              <button onClick={()=> setActiveTab(tab)} key={tab} className={`flex-1 px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${activeTab === tab ? "bg-pink-500 text-white" : "text-gray-600 hover:text-black"}`}>
            ...
            <div className='flex flex-wrap mt-6 max-w-6xl'>
              {
                posts.filter((post)=>post.image_urls.length > 0).map((post)=>(
                  ...
                  {post.image_urls.map((image, index)=>(
                    ...
                    <img src={image} key={index} className='w-64 aspect-square object-cover' alt=''/>
