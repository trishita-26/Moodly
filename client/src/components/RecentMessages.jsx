                ...
                messages.map((message,index)=>(
                    <Link to={`/messages/${message.from_user_id._id}`} key={index} className='flex items-start gap-2 py-2 hover:bg-slate-100'>
                        ...
                        <div className='w-full'>
                            ...
                            <div className='flex justify-between'>
                                <p className='text-shadow-gray-600'>{message.text? message.text : 'Media'}</p>
                                {!message.seen && <p className='bg-rose-400 text-white w-4 h-4 flex items-center justify-center rounded-full text-[10px]'>1</p>}
                            </div>
